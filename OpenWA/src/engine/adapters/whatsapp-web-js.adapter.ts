...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  async initialize(callbacks: EngineEventCallbacks): Promise<void> {
    ...
    try {
      ...
      // Clear Chromium SingletonLock and SingletonSocket files to prevent startup failure on container restart
      ...
      for (const lock of locks) {
        try {
          unlinkSync(lock);
          this.logger.log(`Cleared stale Chromium lock file: ${lock}`);
        } catch (err: any) {
          if (err?.code !== 'ENOENT') {
            this.logger.warn(`Could not clear lock file ${lock}`, { error: String(err) });
          }
        }
      }
  ...
  private setupEventHandlers(): void {
    ...
    this.client.on('loading_screen', (percent: number, message: string) => {
      this.logger.log(`WhatsApp Web Loading: ${percent}% - ${message}`, { percent, message });
    });

    this.client.on('change_state', (state: string) => {
      this.logger.log(`WhatsApp Web state changed: ${state}`, { state });
    });
    ...
    this.client.on('authenticated', () => {
      ...
      // Start active polling for client info in case the 'ready' event is delayed
      const checkInterval = setInterval(() => {
        ...
        try {
          ...
          if (info && info.wid) {
            ...
            this.logger.log(`Session transitioned to READY via active polling: ${this.phoneNumber}`);
          }
        } catch {
          // ignore until ready
        }
    ...
    this.client.on('auth_failure', (msg: string) => {
      this.logger.error('WhatsApp Web Authentication Failure', msg);
      this.setStatus(EngineStatus.FAILED);
      this.callbacks.onStateChanged?.(EngineStatus.FAILED);
    });

    this.client.on('disconnected', (reason: string) => {
      this.logger.warn(`WhatsApp Web Disconnected: ${reason}`, { reason });
    ...
    this.client.on('ready', () => {
      try {
        ...
        this.setStatus(EngineStatus.READY);
        this.callbacks.onReady?.(this.phoneNumber || '', this.pushName || '');
        this.logger.log(`Session transitioned to READY via ready event: ${this.phoneNumber}`);
      } catch (error) {
  ...
  private async resolveChatId(chatId: string): Promise<string> {
    ...
    // Strip out device suffixes like :0 or :1
    let clean = chatId.replace(/:\d+@/, '@');
    if (clean.endsWith('@s.whatsapp.net')) {
      clean = clean.replace('@s.whatsapp.net', '@c.us');
    }
    ...
    if (this.client && cleanNumber) {
      try {
        ...
        this.logger.warn(`Could not resolve numberId for ${cleanNumber}: ${err?.message || err}`);
      }
    }
  ...
  async sendTextMessage(chatId: string, text: string): Promise<MessageResult> {
    ...
    this.logger.log(`Attempting to send text message to: ${targetChatId}`);
    try {
      const msg = await this.client!.sendMessage(targetChatId, text);
      if (!msg) {
        // Some WhatsApp Web builds resolve sendMessage after accepting the
        // outbound message but return no Message object. Treating that as a
        // hard failure produced a false 500 even though recipients received
        // the OTP. A rejected/failed send still throws and is handled below.
        this.logger.warn(
          `Message accepted without receipt metadata for ${targetChatId}`,
        );
        return {
          id: `accepted_${targetChatId}_${Date.now()}`,
          timestamp: Math.floor(Date.now() / 1000),
        };
      }
      const id = msg.id?._serialized || msg.id?.id || (typeof msg.id === 'string' ? msg.id : '') || `true_${targetChatId}_${Date.now()}`;
      return {
        id,
        timestamp: msg.timestamp || Math.floor(Date.now() / 1000),
      };
    } catch (err: any) {
      this.logger.error(`Failed to send message to ${targetChatId}: ${err?.message || err}`, err?.stack);
      throw err;
    }
  }
  ...
  private async sendMediaMessage(chatId: string, media: MediaInput): Promise<MessageResult> {
    ...
    if (!msg || !msg.id) {
      throw new Error(`Failed to send media message: WhatsApp Web could not deliver to chat '${chatId}'`);
    }
  ...
  private ensureReady(): void {
    ...
    // If client is already authenticated with info, allow operations

...
export class AuthService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.log('');
    // Never write credentials to stdout: container logs are commonly exported
    // to central logging systems. Keys are managed through the dashboard/secret
    // store and the administrator is told only where to retrieve them.
    if (isNewKey) {
      this.logger.warn('  🔑 A new API key was created. Retrieve it from the secure key store/dashboard.');
    } else {
      this.logger.log('  🔑 API keys are managed through the secure key store/dashboard.');
    }
    this.logger.log('');
    this.logger.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    this.logger.log('');
  ...
  async validateApiKey(rawKey: string, clientIp?: string, sessionId?: string): Promise<ApiKey> {
    ...
    // Never log raw credentials. Container logs are routinely exported and
    // previously exposed both the presented key and the master key here.
    if (masterKey && key === masterKey) {
      const virtualKey = new ApiKey();
      virtualKey.id = '00000000-0000-0000-0000-000000000000';
      virtualKey.name = 'Master API Key';
      virtualKey.role = ApiKeyRole.ADMIN;
      virtualKey.isActive = true;
      virtualKey.allowedIps = null;
      virtualKey.allowedSessions = null;
      virtualKey.usageCount = 0;
      virtualKey.lastUsedAt = new Date();
      return virtualKey;
    }

...
## 7-ب. الاختبار الميداني اليدوي — **[قسم جديد 2026-09-12]**

> يكمّل هذا القسم بوابات §7 الآلية بسيناريوهات لا يمكن أتمتتها بالكامل في CI (أجهزة حقيقية، انقطاع شبكة فعلي، حجم بيانات كبير، تعدد أجهزة). يُنفَّذ يدويًا على بيئة staging قبل أي تجريبي مغلق حقيقي مع محلات فعلية.

### السيناريوهات الإلزامية (لا يُطلق التجريبي المغلق قبل اجتيازها كلها)
1. دفتر محلي في وضع الطيران (بلا حساب، من الصفر).
2. إغلاق التطبيق قسرًا فور تسجيل دين — إعادة الفتح لا يفقده.
3. نقل دفتر محلي إلى حساب سحابي مع قطع الشبكة في منتصف النقل.
4. تاجر وعميل على جهازين منفصلين يتفاعلان (ربط، تأكيد، اعتراض) في وقت متقارب.
5. بقالتان مختلفتان لنفس رقم العميل.
6. عملتان مختلفتان مفعّلتان في نفس البقالة.
7. دورة تأكيد قيد ثم اعتراض عليه ثم رد التاجر.
8. عضو فريق لكل دور من الأدوار الستة يجرّب عملية مسموحة وأخرى ممنوعة.
9. فقد/انتهاء جلسة Supabase أثناء استخدام فعلي — لا فقد بيانات محلية.
10. توقف حاوية OpenWA فجأة — مراقبة تكتشفه، ومسار OTP يفشل برسالة واضحة لا بصمت.
11. تغيير كلمة السر من الاسترداد ثم الدخول بالجديدة.
12. استعادة نسخة احتياطية على جهاز جديد كليًا.
13. ترقية تثبيت قديم (قاعدة بيانات محلية غير مشفرة) إلى النسخة الجديدة — لا فقد أرصدة.
14. حجم واقعي: ٥٠٠٠ عميل و٢٠٠٠٠ قيد على حساب واحد — الأداء يبقى مقبولاً (فتح الشاشة، البحث، التمرير).
15. عمليتان متزامنتان من جهازين مختلفين على نفس العميل في نفس اللحظة تقريبًا — لا تكرار ولا تضارب صامت في الرصيد.

### بوابة القبول
لا ينتقل التطبيق إلى تجريبي مغلق مع محلات حقيقية عند وجود أي عطل من نوع: فقد بيانات، تكرار عملية مالية، تسرّب بيانات بين حسابين مختلفين، اختلاف رصيد بين العرض المحلي والسيرفر، تجاوز صلاحيات دور، تسجيل حساب دون تحقق حقيقي عبر OpenWA، أو فشل استعادة النسخة الاحتياطية. أي عطل من هذه القائمة = No-Go بصرف النظر عن نجاح بقية السيناريوهات.

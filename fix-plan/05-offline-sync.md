...
- **حالة 2026-09-12 — الفقرات 1 و4 منفَّذتان:**
  ...
  - **حد معروف غير قابل للتجاوز من هذه البيئة:** `sqflite_sqlcipher` بلا دعم Linux/Windows — التحقق الفعلي من فتح/ترقية قاعدة مشفَّرة حقيقية يتطلب جهاز/محاكي Android أو iOS/macOS فعلياً؛ ما نُفِّذ هنا مُتحقَّق منه بـ`flutter analyze`/`flutter test` (41 اختباراً ناجحاً) لا بتشغيل SQLCipher الحقيقي.
  - **لم يُنفَّذ بعد (الفقرتان 2 و3):** قفل التطبيق (PIN/بصمة/مهلة) والنسخ الاحتياطي المحلي المشفَّر بكلمة مرور — لا تزالان معلَّقتين ضمن هذه الخطوة.

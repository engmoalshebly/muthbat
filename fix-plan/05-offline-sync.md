...
**الخطوة 12-ب — تشفير التخزين المحلي وقفل التطبيق [نُقلت من مرحلة 2 وُوسِّعت — 2026-09-12].**
- **لماذا نُقلت إلى هنا:** كانت هذه الخطوة (سابقًا 17) مصنّفة "مرحلة 2 — إطلاق تجاري"، أي بعد التجريبي المغلق. لكن دفتر الضيف المحلي وكاش الحساب يحتويان أسماء عملاء وأرصدة مالية فعلية منذ أول استخدام — تأجيل التشفير لما بعد التجريبي يعني تشغيل بيانات مستخدمين حقيقيين بلا حماية. صُنِّفت الآن **يحظر الإطلاق**.
- **الملفات:** `mobile/lib/core/database/app_database.dart` (دفتر الضيف، كاش الحساب، طابور المزامنة)، تخزين رقم الهاتف المحلي، آلية النسخ الاحتياطي المحلي.
- **التغيير المطلوب:**
  1. الانتقال إلى قاعدة مشفّرة (`sqflite_sqlcipher` أو ما يعادلها) بمفتاح مشتق ومخزَّن في Android Keystore / iOS Keychain — لا مفتاح ثابت في الكود.
  2. قفل تطبيق اختياري: PIN + بصمة (اختياري) + مهلة قفل تلقائي بعد عدم النشاط.
  3. تشفير أي نسخة احتياطية محلية بكلمة مرور يحددها المستخدم (AES-GCM مع اشتقاق مفتاح موثق — لا تشفير مخصص).
  4. مسار ترقية آمن لقواعد بيانات قديمة غير مشفرة: تحويلها للتنسيق المشفر عند أول تشغيل بعد الترقية دون فقد أي رصيد أو قيد.
  5. عدم كتابة أي بيانات حساسة (اسم عميل، رصيد، رقم هاتف) في سجلات التطبيق (`debugPrint`/logs).
- **معيار التحقق:** نسخ ملف قاعدة البيانات مباشرة وقراءته بأداة SQLite عادية يفشل؛ نسخة احتياطية بلا كلمة المرور الصحيحة غير قابلة للقراءة؛ ترقية مستخدم قديم (قاعدة غير مشفرة) تنجح وتحافظ على كل الأرصدة والقيود؛ قفل التطبيق لا يمنع استمرار حفظ العمليات المعلقة في الطابور.
- **الاعتماديات:** لا شيء تقني، لكن يُنسَّق مع خطة الأمان (`03-security-secrets.md`) لتناسق إدارة المفاتيح.
- **الجهد:** 3 يوم (بدل 1 يوم في التقدير الأصلي — يشمل الترقية الآمنة وقفل التطبيق، لا التشفير وحده).
- **حالة 2026-09-12 — الفقرات 1–4 منفَّذة:**
  - `AppDatabase` (دفتر الحساب) و`LocalLedgerStore` (دفتر الضيف) تفتحان الآن عبر `sqflite_sqlcipher` بمفتاح 256-بت مولَّد عشوائياً ومخزَّن في `flutter_secure_storage` (Keystore/Keychain)، ملف مفتاح لكل قاعدة بيانات — `lib/core/database/db_encryption.dart`.
  - مسار ترقية آمن ومُتحقَّق منه: `lib/core/database/sqlcipher_migration.dart` يكتشف ملفاً قديماً غير مشفَّر (فحص ترويسة "SQLite format 3" الخام لا محاولة فتح)، يهاجره عبر `sqlcipher_export()` الرسمية، **يقارن عدد صفوف كل جدول قبل/بعد** ويرفض حذف الأصل عند أي اختلاف، ويُبقي نسخة `*.pre-encryption.bak` بدل حذف فوري. اختبارات `test/sqlcipher_migration_test.dart` تغطي منطق الفحص والحالات الآمنة (ملف غير موجود، ملف مشفَّر بالفعل).
  - إضافة قاعدة ProGuard في `android/app/proguard-rules.pro` (`-keep class net.sqlcipher.**`) استباقياً — التصغير `minifyEnabled` غير مفعَّل بعد في هذا المستودع، فالقاعدة بلا أثر حالياً لكنها ستمنع كسر SQLCipher صامتاً حين يُفعَّل ضمن 09-28.
  - **حد معروف غير قابل للتجاوز من هذه البيئة:** `sqflite_sqlcipher` بلا دعم Linux/Windows — التحقق الفعلي من فتح/ترقية قاعدة مشفَّرة حقيقية يتطلب جهاز/محاكي Android أو iOS/macOS فعلياً؛ ما نُفِّذ هنا مُتحقَّق منه بـ`flutter analyze`/`flutter test` لا بتشغيل SQLCipher الحقيقي.
  - **الفقرة 3 (النسخ الاحتياطي المشفَّر) منفَّذة أيضاً:** `LocalLedgerStore.backup({password})`/`restore(text, {password})` يغلّفان النسخة عبر `lib/core/security/encrypted_backup.dart` (`EncryptedBackupCodec`) — AES-256-GCM حقيقي عبر `package:cryptography`، بمفتاح مشتق من كلمة مرور المستخدم عبر PBKDF2-HMAC-SHA256 بـ600,000 تكرار (توصية OWASP الحالية)، وملح عشوائي لكل نسخة. لا تُقرأ النسخة بلا كلمة المرور الصحيحة (فشل مصادقة AEAD يُترجَم لخطأ عربي واضح). النسخ القديمة غير المشفَّرة تبقى قابلة للاستعادة بلا كلمة مرور (توافق خلفي). الواجهة (`local_ledger_screen.dart`): مربع حوار عند إنشاء النسخة يسأل عن حماية اختيارية بكلمة مرور (6 أحرف فأكثر)، ومربع حوار عند الاستعادة يطلب كلمة المرور فقط إذا اكتشف أن الملف مشفَّر. اختبارات: `test/encrypted_backup_test.dart` (الترميز نفسه: دوران كامل، كلمة مرور خاطئة، تلاعب بالنص المشفَّر، صيغة غير مدعومة) + اختبار تكامل في `test/local_ledger_store_test.dart` (نسخة محمية تُستعاد على جهاز جديد، وتُرفض بلا كلمة مرور أو بكلمة خاطئة).
  - **الفقرة 2 منفَّذة:** قفل اختياري للتطبيق برمز من 6 أرقام؛ لا يُخزَّن الرمز نفسه بل PBKDF2-HMAC-SHA256 بملح عشوائي داخل Keystore/Keychain. يدعم بصمة/Face ID اختيارية، وقفلًا فورياً أو بعد 30 ثانية/دقيقة/5 دقائق، وقفلًا مؤقتًا بعد خمس محاولات خاطئة. حالة التفعيل مشتقة من وجود التجزئة المحمية وليست من إعداد قابل للتعديل. شاشة الإعداد متاحة للدفتر المحلي والتاجر والزبون، وغطاء معتم يمنع ظهور البيانات قبل اكتمال فحص القفل عند التشغيل. الاختبارات تغطي التحقق والتعطيل والمدد والقفل بعد المحاولات؛ يلزم قبول البصمة ودورة الخلفية/الاستئناف على جهاز Android وiOS فعليين.
...
**الخطوة 17 — [نُقلت إلى «مرحلة صفر» كخطوة 12-ب موسَّعة — 2026-09-12] تشفير قاعدة SQLite.**
- راجع الخطوة 12-ب أعلاه — لم تعد هذه الخطوة جزءاً من مرحلة الإطلاق التجاري.

...
## 2. إعادة تصميم OTP كاملاً على الخادم [مرحلة صفر — يحظر الإطلاق] — **[منفَّذ فعليًا عبر OpenWA]**

> **تصحيح 2026-09-12:** `fix-plan.md` (القرار 3، 2026-08-18) كان قد حسم إلغاء هذا القسم بالكامل لصالح Twilio Verify. صاحب المشروع اعتمد لاحقًا **OpenWA إلزاميًا**، فهذا القسم هو فعليًا **ما نُفِّذ** (`send-whatsapp-otp`/`verify-whatsapp-otp` + جدول تحدٍّ خادمي + `enforceRateLimits`) — راجع `04-auth-system.md` القرار 1 المصحَّح للتفاصيل الحالية الدقيقة. فارق واحد عن التصميم الأصلي أدناه: التنفيذ الفعلي **لا يستخدم `phoneProofToken`** — بعد نجاح `verify-whatsapp-otp` تُنشئ/تُحدّث الدالة المستخدم مباشرة عبر Admin API (`createUser`/`updateUserById`)، ثم يستدعي العميل `signInWithPassword` الأصلي لفتح الجلسة. بقية هذا القسم (مخطط التدفق، الترحيل، الخطوات) يبقى مرجعًا تاريخيًا صحيحًا في جوهره لمن يراجع القرار.
